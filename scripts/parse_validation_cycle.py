...
def run():
    description = 'Parses TON validators JSON and returns result.'
    ...
    log.log(os.path.basename(__file__), 3, "Looking for active cycle")
    dt = datetime.datetime.now(timezone.utc)
    now = dt.replace(tzinfo=timezone.utc).timestamp()
    cycle = None
    for record in data:
    ...
    if not cycle:
        log.log(os.path.basename(__file__), 1, "Could not find current validation cycle")
        sys.exit(1)

    log.log(os.path.basename(__file__), 3, "Looking for ADNL '{}'".format(args.adnl[0]))
    adnl_data = next((chunk for chunk in cycle["cycle_info"]["validators"] if chunk["adnl_addr"] == args.adnl[0]), None)
    if not adnl_data:
